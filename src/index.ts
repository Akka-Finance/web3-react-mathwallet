import detectEthereumProvider from '@akkafinance/mathwallet-detect-provider';
import type {
    Actions,
...
type MathWalletProvider = Provider & { isMathWallet?: boolean; isConnected?: () => boolean; providers?: MathWalletProvider[] }
...
export class MathWallet extends Connector {
    ...
    public provider?: MathWalletProvider

    private readonly options?: Parameters<typeof detectEthereumProvider>[0]
    private eagerConnection?: Promise<void>

    constructor({ actions, options, onError }: MathWalletConstructorArgs) {
    ...
    private async isomorphicInitialize(): Promise<void> {
        ...
        return (this.eagerConnection = import('@akkafinance/mathwallet-detect-provider').then(async (m) => {
            const provider = await m.default(this.options)
            if (provider) {
                this.provider = provider as unknown as MathWalletProvider

                if (this.provider.providers?.length) {
                    this.provider = this.provider.providers.find((p) => p.isMathWallet) ?? this.provider.providers[0]
                }
